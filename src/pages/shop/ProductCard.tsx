...
const ProductCard: React.FC<ProductProps> = ({ currentProduct, isVisualizer }) =>  {
  ...
  //Firestore Reference
  const productRef = doc(db, `users/${user?.uid}/shops/${currentShop.uid}/products`, currentProduct.uid) 
  ...
  async function toggleProductStatus(){
    ...
    try{   
      ...
      ? dispatch(setCurrentShopPendingProducts(currentShopPendingProducts.filter((product) => product.uid != changedProduct.uid)))
      :  dispatch(setCurrentShopCartProducts(currentShopCartProducts.filter((product) => product.uid != changedProduct.uid)))
      
      newProductStatus === true 
        ?  dispatch(setCurrentShopCartProducts(currentShopCartProducts.concat(changedProduct)))
        : dispatch(setCurrentShopPendingProducts(currentShopPendingProducts.concat(changedProduct)))

      toast({
        variant: "success",
        title: "Sucesso!",
        description: newProductStatus ? "Produto adicionado ao carrinho" : "Produto removido do carrinho",
      })
    }
    catch (error) {
      toast({
  ...
  async function removeProduct(){
    try{
      ...
      currentProduct.isDone 
        ?  dispatch(setCurrentShopCartProducts(currentShopCartProducts.filter((product) => product.uid != currentProduct.uid))) 
        : dispatch(setCurrentShopPendingProducts(currentShopPendingProducts.filter((product) => product.uid != currentProduct.uid)))
  ...
  async function editProduct(data: z.infer<typeof ProductEditFormSchema>){
    try{
      ...
      if(productToEdit.isDone){
        dispatch(setCurrentShopCartProducts(
          currentShopCartProducts.map((element) =>  element.uid === productToEdit.uid ? {...element, ...productToEdit} : element))
        ) 
      } else {
        dispatch(setCurrentShopPendingProducts(
          currentShopPendingProducts.map((element) =>  element.uid === productToEdit.uid ? {...element, ...productToEdit} : element)
        ))
      }
    ...
    <div className='flex flex-row w-[316px] min-h-[62px] justify-between bg-secondary py-3 px-4 rounded-2xl border border-accent shadow gap-2'>
      <section className='flex flex-row items-center gap-2'>
        ...
        <div className='flex flex-col'>
          ...
          <span className="text-xs text-slate-400 break-all ...">{currentProduct.description}</span>
        </div>
      </section>

      <section className='flex flex-row items-center gap-1.5'>
        <span className="text-xs text-black font-bold">{currentProduct.quantity}x</span>
        <span className="text-xs text-black font-bold">{formatPrice(currentProduct.price)}</span>
        {
          !isVisualizer && (
