...
interface AddProductsFormProps{
  ...
  onProductsAdd: (data: z.infer<typeof ProductsCreateFormSchema>, form: UseFormReturn<{text: string;}>) => void;
}

const AddProductsForm: React.FC<AddProductsFormProps> =  ({createProductsLoading, onProductsAdd}) => {
  ...
  return (
    <footer className='w-80 sticky bottom-0 flex justify-center py-2'>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleAddProducts)} className="flex flex-col items-center justify-between w-full space-y-2">
